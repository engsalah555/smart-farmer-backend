...
class MigrateOldMedia extends Command
{
    ...
    public function handle()
    {
        ...
        Store::all()->each(function ($store) {
            if (\Illuminate\Support\Facades\Schema::hasColumn('stores', 'logo') && $store->logo && $store->getMedia('logo')->isEmpty()) {
                $this->addMediaFromUrl($store, $store->logo, 'logo');
            }
            if (\Illuminate\Support\Facades\Schema::hasColumn('stores', 'cover_image') && $store->cover_image && $store->getMedia('cover')->isEmpty()) {
        ...
        StoreCatalog::all()->each(function ($catalog) {
            if (\Illuminate\Support\Facades\Schema::hasColumn('store_catalogs', 'image_url') && $catalog->image_url && $catalog->getMedia('catalog_image')->isEmpty()) {
        ...
        Product::all()->each(function ($product) {
            if (\Illuminate\Support\Facades\Schema::hasColumn('products', 'image_url') && $product->image_url && $product->getMedia('main_image')->isEmpty()) {
